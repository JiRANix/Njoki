                ...
                if (class_exists($decorator)) {
                    ...
                    if ($request->payment_option=='mpesa'){
                        ...
                        if ($request->user()->phone !=null){
                            ...
                            $phone ='254'. substr($request->user()->phone, -9);

                            $response= MpesaProvider::stk_push($phone,intval($combined_order->grand_total), $combined_order->id, "Payment",$request->user()->id);

                            Log::info($response);

                            flash(translate('Your order has been placed successfully. Please complete payment via stk push'))->success();
                            return redirect()->route('home');

                        }
                        flash(translate('please update you mpesa phone number'))->success();
